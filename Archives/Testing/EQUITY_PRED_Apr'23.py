...
historical1 = get_nifty_historicals('2d','1m')
...
for i in range(backcandles,len(historical)-future_candles):
    ...
    y_var = 0
    if (max(Y_var_high) -  Y_var_high[0])> 500:
        y_var = 1
    if  (Y_var_low[0] - min(Y_var_low)) > 500:   
        y_var = 2
    Y.append(y_var)
...
zero_sampled_working_data_set = working_data_set[working_data_set["Y"] == 0].sample(n=int(round(len(working_data_set[working_data_set["Y"] == 0])*.02,0)), random_state=14)
...
for i in range(backcandles,len(historical1)-future_candles):
    ls = historical1.iloc[i-backcandles:i]['Open'].tolist()
    transformed.append([x/ ls[0] for x in ls])
    
    Y_var_high = historical1.iloc[i:i+future_candles]['High'].tolist()
    Y_var_low = historical1.iloc[i:i+future_candles]['Low'].tolist()
    y_var = 0
    if (max(Y_var_high) -  Y_var_high[0])> 200:
        y_var = 1
    if  (Y_var_low[0] - min(Y_var_low)) > 200:   
        y_var = 2
    Y.append(y_var)
...
x = working_data_set.drop(["Y"],axis=1)
y = working_data_set[['Y']]
...
pd.concat([y.reset_index(drop=True),pd.DataFrame(y_pred)],axis= 1).to_clipboard()

score = []
for depth in [1,5,15,20] : 
...
y_pred = reg.predict(X_train).astype(int)
